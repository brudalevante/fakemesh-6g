...
var HearingMap = form.DummyValue.extend({
	renderWidget: function () {
		...
		for (var mac in Clients) {
			...
			body.appendChild(E('h4', maciphost));
...
function tootltip(mac, IP, hostname) {
	...
	if (typeof IP !== 'undefined') {
		for (var IPaddr in IP['ipaddrs'])
			body.appendChild(E('div', '%h'.format(IP['ipaddrs'][IPaddr])));
		for (var IPaddr in IP['ip6addrs'])
			body.appendChild(E('div', '%h'.format(IP['ip6addrs'][IPaddr])));
	};
	if (hostname !== '') {
			...
			if (typeof Clients[mac] !== 'undefined')
				if (typeof Clients[mac][wlan] !== 'undefined')
					if (String(Clients[mac][wlan]['connected']).valueOf() === 'true') {
						...
						hostl.appendChild(E('span', { 'class': 'cbi-tooltip-container' }, [
							'%h\u2003'.format(foundname),
...
function collectRemoteHosts(remotehosttableentries, Remotehosts) {
	const getUndefinedDnsCacheIPs = (Remotehosts, dns_cache) => Object.keys(Remotehosts).filter(IPaddr => !dns_cache.hasOwnProperty(IPaddr));
	...
	L.resolveDefault(callNetworkRrdnsLookup(ipAddrs, 1000, 1000), {}).then(function (replies) {
		for (var address of ipAddrs) {
			...
			if (replies[address]) {
				...
				dns_cache[address] = Hosts[Object.keys(Hosts).find(mac => ((typeof Hosts[mac]['name'] !== 'undefined') && ((Object.keys(Hosts[mac]['ip6addrs']).find(IPaddr2 => (address === Hosts[mac]['ip6addrs'][IPaddr2]))) || (Object.keys(Hosts[mac]['ipaddrs']).find(IPaddr2 => (address === Hosts[mac]['ipaddrs'][IPaddr2]))))))]['name'];
			}
		}
	});
...
var Clientinfooverview = form.DummyValue.extend({
	renderWidget: function () {
		...
		body.appendChild(E('h3', _('Client list')));
...
var Settingstitle = form.DummyValue.extend({
	renderWidget: function () {
		var body = E([
			E('h3', _('Settings')),
			E('div', _('The first four options below are mandatory.') + ' ' +
...
return view.extend({
	...
	callGetRemotehosts: rpc.declare({ object: 'usteer', method: 'remote_hosts', expect: { '': {} } }),
	callGetRemoteinfo: rpc.declare({ object: 'usteer', method: 'remote_info', expect: { '': {} } }),
	callGetLocalinfo: rpc.declare({ object: 'usteer', method: 'local_info', expect: { '': {} } }),
	callGetClients: rpc.declare({ object: 'usteer', method: 'get_clients', expect: { '': {} } }),
	...
	render: function (data) {
		var m, s, o;
		if (!('usteer2' in data[0])) {
			m = new form.Map('usteer2', _('Usteer2'),
		...
		o.placeholder = 3;
		o.datatype = 'uinteger';
		o = s.taboption('settings', form.Value, 'roam_scan_timeout', _('Roam scan timeout'), _('Retry scanning when roam_scan_tries is exceeded after this timeout (in ms).') +
		...
		o.placeholder = -60;
		o.datatype = 'integer';
		o = s.taboption('settings', form.Value, 'link_measurement_interval', _('Link measurement interval'), _('Interval (ms) the device is sent a link-measurement request to help assess the bi-directional link quality.') +
