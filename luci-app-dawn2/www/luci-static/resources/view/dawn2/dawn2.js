...
var NetworkOverview = form.DummyValue.extend({
    renderWidget: function () {
        ...
        let client_table = {};
        var body = E([E('h3', _('Network Overview'))]);

        // Tabla normal para cada SSID (todas las bandas)
        for (let network in dawnNetworkData) {
            body.appendChild(E('h4', 'SSID: ' + network));
        ...
        // ===========================
        // Tabla especial solo APs de 6GHz (WiFi 6E/7) para cada SSID
        // ===========================
        body.appendChild(E('h3', _('Access Points in 6 GHz (WiFi 6E/7)')));
        for (let network in dawnNetworkData) {
            // Filtrar solo APs en frecuencia 6GHz
            let aps6g = Object.entries(dawnNetworkData[network])
                .filter(function ([mac, ap]) {
                    return ap.freq >= 5925 && ap.freq <= 7125;
                })
                .map(function ([mac, ap]) {
                    client_table[mac] = E('table', { 'class': 'table cbi-section-table', 'style': 'display: table' }, [
                        E('tr', { 'class': 'tr table-titles' }, [
                            E('th', { 'class': 'th' }, _('Client')),
                            E('th', { 'class': 'th' }, E('span', { 'data-tooltip': _('High Throughput') }, [_('HT')])),
                            E('th', { 'class': 'th' }, E('span', { 'data-tooltip': _('Very High Throughput') }, [_('VHT')])),
                    ...
                    cbi_update_table(client_table[mac], clients, E('em', _('No clients connected.')));
                    return [
                        ap.hostname,
                        ap.iface,
                        mac,
                        dawn.getFormattedNumber(ap.channel_utilization, 2, 2.55) + '%',
                        dawn.getFormattedNumber(ap.freq, 3, 1000) + ' GHz (' + _('Channel') + ': ' + dawn.getChannelFromFrequency(ap.freq) + ')',
                        ap.num_sta,
                        dawn.getAvailableText(ap.ht_support),
                        dawn.getAvailableText(ap.vht_support),
                        ap.num_sta > 0 ? client_table[mac] : E('em', { 'style': 'display: inline' }, _('No clients connected.'))
                    ];
                });
            if (aps6g.length > 0) {
                let ap_table6g = E('table', { 'class': 'table cbi-section-table' }, [
                    E('tr', { 'class': 'tr table-titles' }, [
                        E('th', { 'class': 'th left cbi-section-actions' }, _('Access Point')),
                        E('th', { 'class': 'th left cbi-section-actions' }, _('Interface')),
                        E('th', { 'class': 'th left cbi-section-actions' }, _('MAC')),
                ...
                cbi_update_table(ap_table6g, aps6g, E('em', _('No access points in 6 GHz band.')));
                body.appendChild(E('h4', network));
                body.appendChild(ap_table6g);
            }
        }
...
return view.extend({
    ...
    load: function () {
        return Promise.all([
            dawn.isDawnRPCAvailable().then(function (isAvailable) { return (isAvailable ? dawn.callDawnGetHearingMap() : null) }),
            dawn.isDawnRPCAvailable().then(function (isAvailable) { return (isAvailable ? dawn.callDawnGetNetwork() : null) }),
            dawn.callHostHints()
        ]);
    },
