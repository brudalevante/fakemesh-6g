...
agent_keep_alive()
{
	...
	while :; do
		sleep 10
		sta="$(ubus -S list wpa_supplicant.meshx1)" # 5GHz STA
		if test -n "$sta"; then
...
if [ "$role" = "controller" ]; then
	...
	if test -n "$wbandstr"; then
		...
		if [ "$fronthaul_disabled" = "1" ]; then
			uci set wireless.meshx0.disabled='0'
		else
	...
	lan_ports="$(uci get network.@device[$lan_idx].ports 2>/dev/null)"
	wan_port="$(uci get network.wan.device 2>/dev/null)"
	echo "$lan_ports" | grep -q "$wan_port" || {
		uci add_list network.@device[$lan_idx].ports="$wan_port"
	}
	...
	while uci get firewall.@zone[$lan_idx] &>/dev/null && [ $(uci get firewall.@zone[$lan_idx].name) != "lan" ]; do lan_idx=$((lan_idx+1)); done
	lan_net="$(uci get firewall.@zone[$lan_idx].network 2>/dev/null)"
	echo "$lan_net" | grep -q "meshx0" || {
		uci add_list firewall.@zone[$lan_idx].network="meshx0"
	}

	#setup meshx0
	uci set network.meshx0=interface
	uci set network.meshx0.proto='static'
	uci set network.meshx0.ipaddr="${access_ip}"
	uci set network.meshx0.netmask='255.255.255.0'
	uci set network.meshx0.auto='1'

	#setup meshx0_6 for ipv6
	...
	uci set network.meshx0_6.ifname='meshx0'
	uci set network.meshx0_6.auto='1'

	#setup dhcp for meshx0
	uci set dhcp.meshx0=dhcp
	uci set dhcp.meshx0.interface='meshx0'
	uci set dhcp.meshx0.start='100'
	uci set dhcp.meshx0.limit='150'
	uci set dhcp.meshx0.leasetime='12h'
	uci set dhcp.meshx0.dhcpv6='server'
	uci set dhcp.meshx0.ra='server'
	uci set dhcp.meshx0.ra_management='1'
	uci set dhcp.meshx0.ra_default='1'

	#setup firewall for meshx0
	uci set firewall.fakemesh=rule
	uci set firewall.fakemesh.name='Allow-Fakemesh'
	uci set firewall.fakemesh.src='meshx0'
	uci set firewall.fakemesh.dest_port='65535'
	uci set firewall.fakemesh.proto='tcp'
	uci set firewall.fakemesh.target='ACCEPT'

	#add controller.fakemesh to /etc/hosts
	echo "${access_ip} controller.fakemesh" >>/etc/hosts

	#add firewall.include
	echo "sh /usr/share/fakemesh/firewall.include" >>/etc/firewall.include

	#setup firewall.include
	cat >/usr/share/fakemesh/firewall.include <<EOF
# Allow meshx0 to access lan
iptables -I forwarding_rule -i meshx0 -o br-lan -j ACCEPT
iptables -I forwarding_rule -i br-lan -o meshx0 -j ACCEPT
# Allow meshx0 to access wan
iptables -I forwarding_rule -i meshx0 -o wan -j ACCEPT
iptables -I forwarding_rule -i wan -o meshx0 -j ACCEPT
EOF

	#setup wifi for meshx1 (5GHz) and meshx2 (6GHz)
	for band in 5g 6g; do
		wbandstr=$(uci show wireless | grep .band=\'${band}\'$ -m1)
		if test -n "$wbandstr"; then
			radio=$(echo $wbandstr | cut -d\. -f2)
			meshif="meshx1"
			[ "$band" = "6g" ] && meshif="meshx2"
			
			uci set wireless.$meshif=wifi-iface
			uci set wireless.$meshif.device="${radio}"
			uci set wireless.$meshif.network='lan'
			uci set wireless.$meshif.ifname="$meshif"
			uci set wireless.$meshif.mode='ap'
			uci set wireless.$meshif.wds='1'
			uci set wireless.$meshif.ssid="${Xid}"
			uci set wireless.$meshif.hidden='1'
			uci set wireless.$meshif.wpa_group_rekey='601'
			uci set wireless.$meshif.max_inactivity='301'
			if test -n "${Xkey}"; then
				uci set wireless.$meshif.encryption='psk2'
				uci set wireless.$meshif.key="${Xkey}"
			else
				uci set wireless.$meshif.encryption='none'
			fi
			if [ "$fronthaul_disabled" = "1" ]; then
				uci set wireless.$meshif.disabled='0'
			else
				uci delete wireless.$meshif.disabled 2>/dev/null
			fi
		fi
	done

	#setup wifi for meshx0 (2GHz) - already done above

	#setup wifi for meshx1 (5GHz) STA
	wbandstr=$(uci show wireless | grep .band=\'5g\'$ -m1)
	if test -n "$wbandstr"; then
		radio=$(echo $wbandstr | cut -d\. -f2)
		uci set wireless.meshx1_sta=wifi-iface
		uci set wireless.meshx1_sta.device="${radio}"
		uci set wireless.meshx1_sta.network='meshx0'
		uci set wireless.meshx1_sta.ifname='meshx1'
		uci set wireless.meshx1_sta.mode='sta'
		uci set wireless.meshx1_sta.wds='1'
		uci set wireless.meshx1_sta.ssid="${Xid}"
		uci set wireless.meshx1_sta.hidden='1'
		uci set wireless.meshx1_sta.wpa_group_rekey='601'
		uci set wireless.meshx1_sta.max_inactivity='301'
		if test -n "${Xkey}"; then
			uci set wireless.meshx1_sta.encryption='psk2'
			uci set wireless.meshx1_sta.key="${Xkey}"
		else
			uci set wireless.meshx1_sta.encryption='none'
		fi
		uci set wireless.meshx1_sta.disabled='0'
	fi

	#setup wifi for meshx2 (6GHz) STA
	wbandstr=$(uci show wireless | grep .band=\'6g\'$ -m1)
	if test -n "$wbandstr"; then
		radio=$(echo $wbandstr | cut -d\. -f2)
		uci set wireless.meshx2_sta=wifi-iface
		uci set wireless.meshx2_sta.device="${radio}"
		uci set wireless.meshx2_sta.network='meshx0'
		uci set wireless.meshx2_sta.ifname='meshx2'
		uci set wireless.meshx2_sta.mode='sta'
		uci set wireless.meshx2_sta.wds='1'
		uci set wireless.meshx2_sta.ssid="${Xid}"
		uci set wireless.meshx2_sta.hidden='1'
		uci set wireless.meshx2_sta.wpa_group_rekey='601'
		uci set wireless.meshx2_sta.max_inactivity='301'
		if test -n "${Xkey}"; then
			uci set wireless.meshx2_sta.encryption='psk2'
			uci set wireless.meshx2_sta.key="${Xkey}"
		else
			uci set wireless.meshx2_sta.encryption='none'
		fi
		uci set wireless.meshx2_sta.disabled='0'
	fi

	#setup wifi for meshx0 (2GHz) STA
	wbandstr=$(uci show wireless | grep .band=\'2g\'$ -m1)
	if test -n "$wbandstr"; then
		radio=$(echo $wbandstr | cut -d\. -f2)
		uci set wireless.meshx0_sta=wifi-iface
		uci set wireless.meshx0_sta.device="${radio}"
		uci set wireless.meshx0_sta.network='meshx0'
		uci set wireless.meshx0_sta.ifname='meshx0'
		uci set wireless.meshx0_sta.mode='sta'
		uci set wireless.meshx0_sta.wds='1'
		uci set wireless.meshx0_sta.ssid="${Xid}"
		uci set wireless.meshx0_sta.hidden='1'
		uci set wireless.meshx0_sta.wpa_group_rekey='601'
		uci set wireless.meshx0_sta.max_inactivity='301'
		if test -n "${Xkey}"; then
			uci set wireless.meshx0_sta.encryption='psk2'
			uci set wireless.meshx0_sta.key="${Xkey}"
		else
			uci set wireless.meshx0_sta.encryption='none'
		fi
		uci set wireless.meshx0_sta.disabled='0'
	fi
	...
	if test -n "$wbandstr"; then
		...
		uci delete wireless.meshx1_sta.disabled 2>/dev/null
	fi
	...
	if test -n "$wbandstr"; then
		...
		uci delete wireless.meshx2_sta.disabled 2>/dev/null
	fi
	...
	if test -n "$wbandstr"; then
		...
		uci delete wireless.meshx0_sta.disabled 2>/dev/null
	fi

	#setup wifi for meshx1 (5GHz) AP
	wbandstr=$(uci show wireless | grep .band=\'5g\'$ -m1)
	if test -n "$wbandstr"; then
		radio=$(echo $wbandstr | cut -d\. -f2)
		...
		uci set wireless.meshx1.mode='ap'
		uci set wireless.meshx1.wds='1'
		uci set wireless.meshx1.ssid="${Xid}"
		uci set wireless.meshx1.hidden='1'
		uci set wireless.meshx1.wpa_group_rekey='601'
		uci set wireless.meshx1.max_inactivity='301'
		if test -n "${Xkey}"; then
		...
		if [ "$fronthaul_disabled" = "0" ]; then
			uci set wireless.meshx1.disabled='0'
		else
			uci delete wireless.meshx1.disabled 2>/dev/null
		fi
	fi

	#setup wifi for meshx2 (6GHz) AP
	wbandstr=$(uci show wireless | grep .band=\'6g\'$ -m1)
	if test -n "$wbandstr"; then
		radio=$(echo $wbandstr | cut -d\. -f2)
		uci set wireless.meshx2=wifi-iface
		uci set wireless.meshx2.device="${radio}"
		uci set wireless.meshx2.network='meshx0'
		uci set wireless.meshx2.ifname='meshx2'
		uci set wireless.meshx2.mode='ap'
		uci set wireless.meshx2.wds='1'
		uci set wireless.meshx2.ssid="${Xid}"
		uci set wireless.meshx2.hidden='1'
		uci set wireless.meshx2.wpa_group_rekey='601'
		uci set wireless.meshx2.max_inactivity='301'
		if test -n "${Xkey}"; then
			uci set wireless.meshx2.encryption='psk2'
			uci set wireless.meshx2.key="${Xkey}"
		else
			uci set wireless.meshx2.encryption='none'
		fi
		if [ "$fronthaul_disabled" = "0" ]; then
			uci set wireless.meshx2.disabled='0'
		else
			uci delete wireless.meshx2.disabled 2>/dev/null
		fi
	fi
	...
	uci set network.meshx0.ifname='meshx0'
	uci set network.meshx0.auto='1'
